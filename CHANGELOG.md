...
## [3.1.1](https://github.com/johnkattenhorn/bmalph/compare/v3.1.0...v3.1.1) (2026-04-19)

### 📚 Documentation

* **Clarify `.ralphrc` is loaded once at bmalph startup.** Add a callout to `RALPH-REFERENCE.md` warning that mid-run edits to `.ralphrc` do not take effect until `bmalph run` is restarted. Observed during a long productone run — a mid-run flip of `SESSION_CONTINUITY` appeared to improve metrics, but a later audit showed the running process still held the startup-time value. This is pre-existing bmalph behaviour (not a regression); the docs just hadn't named it. No code change.
